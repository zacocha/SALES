...
1.  What is the total amount each customer spent on zomato?
    ...
    select a.userid, a.product_id, sum(b.price) total_amt_Spent from sales a inner join 
...
8.  What is the total orders and amount spent for each member before they became a member?
    ...
    (select a.userid, a.created_date,a.product_id,b.gold_signup_date)c inner join product d on c.product_id=d.product_id)e 
    group by userid; 


9.  In the first one year after a customer joins the gold program (Including their join date) irrespective of what the customer
    has purchased they earn 5 zomato points for every 10 rs spent who earned more 1 or 3 and what was their points earnings 
    in their first year?
...
11. Rank all the transactions for each member whenever they are a zomato gold member for every non gold member 
